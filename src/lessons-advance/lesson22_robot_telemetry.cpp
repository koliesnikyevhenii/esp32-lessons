...
// ---------------------------------------------------------------------------
void updateTiltGuard() {
  float tilt = max(fabsf(pitch), fabsf(roll));

  if (!tiltBlocked && tilt >= TILT_LIMIT) {
    tiltBlocked = true;
    Serial.printf("[GUARD] наклон %.0f град >= %.0f — СТОП\n", tilt, TILT_LIMIT);
    driveStop();                     // глушим сразу, не дожидаясь команды
  } else if (tiltBlocked && tilt <= TILT_RELEASE) {
    tiltBlocked = false;
...
void loop() {
  ...
  // 3) Обратная связь: наклон может сам заглушить моторы.
  updateTiltGuard();
