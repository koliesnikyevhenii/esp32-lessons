...
| **акселерометр** | `mpu.getAccX/Y/Z()` (библиотека `tockn`, значения в g)              |
...
> Актуальные пины всегда в `include/pins.h`: `MPU_SCL` переехал с 33 на 17, когда
> GPIO33 отдали под ШИМ мотора B (урок 21).
...
- **Урок 22** — этот скетч + моторы из урока 21 в одной прошивке
  (`lesson22_robot_telemetry.md`): рулим и одновременно смотрим угол.
