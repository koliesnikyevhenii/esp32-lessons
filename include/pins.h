...
// HC-SR04 — ультразвуковой дальномер (урок 27, дальше автопилот Этапа 7).
// Датчик питается от 5V (VIN платы), а ECHO отдаёт 5-вольтовую логику — пины
// ESP32 НЕ пятивольтовые, поэтому ECHO заводится через делитель 1к/2к
// (см. documentation/lesson27_hcsr04.md). TRIG — обычный выход 3.3 В, датчик
// его понимает.
//
// Пины выбраны так, чтобы не пересекаться с MPU_* и MOTOR_*: на роботе все три
// периферии работают ОДНОВРЕМЕННО.
//   GPIO39 — вход-ONLY (VN), выходом стать физически не может, и это ровно то,
//            что нужно для ECHO. Подтяжек у 34..39 нет — линию всегда держит
//            делитель, так что вход не «висит».
//   GPIO12 — strapping-пин (MTDI): если он ВЫСОКИЙ в момент сброса, чип
//            выберет 1.8 В для флеша и не загрузится. У GPIO12 на сбросе своя
//            внутренняя подтяжка ВНИЗ, а TRIG у HC-SR04 — вход модуля без
//            подтяжки вверх, поэтому связка рабочая. Если плата вдруг перестала
//            стартовать с подключённым датчиком — перенеси TRIG на любой
//            свободный выход (например 16, если DHT11 не подключён).
#define SONAR_TRIG 12  // выход: импульс 10 мкс -> «пикни»
#define SONAR_ECHO 39  // вход (через делитель!): длительность = время полёта

// SG90 — сервопривод под сонар (урок 28: сканирующий дальномер).
// Сервопривод управляется ШИМом 50 Гц (период 20 мс), где ЗНАЧЕНИЕ — это
// ширина импульса 500..2400 мкс, а не скважность в привычном смысле.
//
// ПИТАНИЕ — ТОЛЬКО от 5 В (шина MP1584EN на роботе), НИКОГДА не от 3V3 платы:
// пусковой ток SG90 доходит до ~700 мА, стабилизатор DevKit столько не даёт — будет
// brownout-ресет ровно в момент поворота. Общий GND обязателен.
//
// Свободных ног на DevKit больше нет (0/1/3 — boot и UART0, 6..11 — флеш,
// 36/39 — только вход), поэтому берём номер из базовых уроков: GPIO19 — это
// LED_GREEN из светофора, которого на роботе нет. Важно другое: с группами
// MPU_*, MOTOR_* и SONAR_* пин не пересекается — на роботе они работают вместе.
#define SERVO_PIN 19   // сигнальный (оранжевый/жёлтый) провод SG90
#define CH_SERVO   4   // LEDC-канал: не 0/1/2 (RGB и buzzer) и не 7 (подсветка камеры)
